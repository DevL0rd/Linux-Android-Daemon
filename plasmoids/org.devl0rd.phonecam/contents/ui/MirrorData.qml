...
Item {
    ...
    P5Support.DataSource {
        id: helper
        engine: "executable"
        onNewData: function(source, d) { root.dir = (d.stdout || "").trim(); disconnectSource(source); root.read() }
    }

    function read() {
        if (!root.dir) return
        var xhr = new XMLHttpRequest()
        xhr.open("GET", "file://" + root.dir + "/phonescreen.json")
    ...
    Timer { interval: root.interval; repeat: true; running: root.dir !== ""; onTriggered: root.read() }
    Component.onCompleted: helper.connectSource("printf %s \"$XDG_RUNTIME_DIR/Linux-Android-Daemon\"")
}
